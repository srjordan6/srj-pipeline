...
var twoaiDailyOnly = map[string]bool{
	...
	// twoai_politics_lda added 2026-09-21 for The Politics of AI. Lobbying
	// filings post during the day and cluster at quarter end; once a day
...
func main() {
	...
	if src == "all" {
		...
		// but the GETs.
		// twoai_stocks sits before twoai_build so the day's closes are in the
		// database when the pages that show them are rendered. It is the only
		// stage that sleeps by design - eight requests a minute on the free
	...
	if src == "twoai_company_sites" {
		if err := twoaiCompanySites(db); err != nil {
			fmt.Fprintln(os.Stderr, "twoai_company_sites:", err)
			os.Exit(1)
		}
		return
	}

	if src == "twoai_model_watch" {
		if err := twoaiModelWatch(db); err != nil {
			fmt.Fprintln(os.Stderr, "twoai_model_watch:", err)
			os.Exit(1)
		}
		return
	}
	...
	if src == "twoai_gaps" {
		if err := twoaiGaps(db); err != nil {
			fmt.Fprintln(os.Stderr, "twoai_gaps:", err)
			os.Exit(1)
		}
		return
	}

	if src == "twoai_ma_readings" {
		if err := twoaiMAReadings(db); err != nil {
			fmt.Fprintln(os.Stderr, "twoai_ma_readings:", err)
			os.Exit(1)
		}
		return
	}
	...
	if src == "twoai_learning_readings" {
		if err := twoaiLearningReadings(db); err != nil {
			fmt.Fprintln(os.Stderr, "twoai_learning_readings:", err)
			os.Exit(1)
		}
		return
	}
	...
	if src == "appsec_research" {
		if err := appsecResearch(db); err != nil {
			fmt.Fprintln(os.Stderr, "appsec_research:", err)
			os.Exit(1)
		}
		return
	}

	if src == "twoai_etf_holdings" {
		if err := twoaiEtfHoldings(db); err != nil {
			fmt.Fprintln(os.Stderr, "twoai_etf_holdings:", err)
			os.Exit(1)
		}
		return
	}

	if src == "twoai_politics_lda" {
		if err := twoaiPoliticsLDA(db); err != nil {
			fmt.Fprintln(os.Stderr, "twoai_politics_lda:", err)
			os.Exit(1)
		}
		return
	}

	if src == "twoai_politics_pages" {
		if err := twoaiPoliticsPages(db); err != nil {
			fmt.Fprintln(os.Stderr, "twoai_politics_pages:", err)
			os.Exit(1)
		}
		return
	}

	if src == "twoai_benchmark_readings" {
		if err := twoaiBenchmarkReadings(db); err != nil {
			fmt.Fprintln(os.Stderr, "twoai_benchmark_readings:", err)
			os.Exit(1)
		}
		return
	}

	if src == "twoai_politics_bills" {
		if err := twoaiPoliticsBills(db); err != nil {
			fmt.Fprintln(os.Stderr, "twoai_politics_bills:", err)
			os.Exit(1)
		}
		return
	}

	if src == "twoai_politics_fec" {
		if err := twoaiPoliticsFEC(db); err != nil {
			fmt.Fprintln(os.Stderr, "twoai_politics_fec:", err)
			os.Exit(1)
		}
		return
	}
	...
	if src == "twoai_stocks" {
		if err := twoaiStocks(db); err != nil {
			fmt.Fprintln(os.Stderr, "twoai_stocks:", err)
			os.Exit(1)
		}
		return
	}
...
func intelAIWatch(db *sql.DB) (added int, err error) {
	...
	feeds := []struct{ vendor, url string }{
		...
		// Renamed from AI Safety Institute on 2025-02-14; the old query missed
		// every story that used the new name. Both names, OR works on Google
		// News RSS. First-party publications come in through twoai_vendor_feeds.
		{"UK AI Security Institute (coverage)", "https://news.google.com/rss/search?q=%22AI+Security+Institute%22+OR+%22AI+Safety+Institute%22+UK&hl=en-US&gl=US&ceid=US:en"},
		// Apollo Research publishes no feed (probed 2026-09-21: /feed, /rss.xml,
		// /blog/rss.xml all 404, no feed link in the blog page).
		{"Apollo Research (coverage)", "https://news.google.com/rss/search?q=%22Apollo+Research%22+AI&hl=en-US&gl=US&ceid=US:en"},
...
func twoaiBuild(db *sql.DB) error {
	...
	// Federal lobbying on AI onto confirmed company pages, after every company
	// document above has been written and before the freshness stamp, so the
	// fast path deploys it too. Never fatal. 2026-09-21.
	if err := twoaiPoliticsCompanyPatch(db); err != nil {
		fmt.Println("twoai_politics_companies:", err)
	}

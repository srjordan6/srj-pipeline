...
func twoaiGraph(db *sql.DB, today string) (int, error) {
	...
	nodes := []nrow{
		...
		{"People", one(`SELECT count(*) FROM twoai_pages WHERE kind IN ('person','person-tracked')`),
		...
		{"Benchmarks tracked", one(`SELECT count(*) FROM twoai_pages WHERE kind LIKE 'benchmark%'`),
			"benchmark page", "/ai-ecosystem/research-knowledge-and-learning/"},
		{"State AI bills", one(`SELECT COALESCE(sum((data->>'bill_count')::int),0) FROM twoai_pages WHERE kind='law-state'`),
		...
		{"MCP servers", one(`SELECT COALESCE(max(url_count),0) FROM twoai_pages WHERE path='mcp/index.json'`),
